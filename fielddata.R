library(igraph)
library(MASS)
testgraph<-read.csv("relations bimode simplified max2.csv",sep="\t",header = FALSE)
Edgevalue<-read.csv("sampleedge.csv",header = TRUE,sep = ",")
ful_n_frame<- graph.data.frame(testgraph,directed=FALSE)
...
pgd<-get.shortest.paths(ful_n_frame,sourcematrix[1],s_sort$ix[N],mode="all",output="epath")
pgdl<-length(unlist(pgd))
csvdfn<-cbind(b,s_vertex,pgdl)
write.table(csvdfn,"sampledata.csv",sep=",",append=TRUE,col.names=NA)
}
